...
function latestImageInfo(device) {
  ...
  return {
    ...
    url: clean(latest.DriveURL) || (clean(latest.DriveFileID) ? `https://example.invalid/configure-google-service)}/view` : ''),
  };
}
...
async function deviceFolderInfo(maintenance, device, cache) {
  ...
  return { id: folder.id, url: folder.webViewLink || `https://example.invalid/configure-google-service}` };
}
...
async function createSpreadsheetReport(ctx) {
  ...
  const title = `Checklist mantenimiento GT - ${data.mantenimiento.Cliente || 'Cliente'} - ${String(data.mantenimiento.Fecha || '').slice(0, 10)}`;
  ...
  const url = created.webViewLink || `https://example.invalid/configure-google-service}/edit`;
  ...
  return {
    spreadsheetId: id,
    spreadsheetUrl: url,
    excelUrl: `https://example.invalid/configure-google-service}/export?format=xlsx`,
...
async function createSlidesReport(ctx) {
  ...
  const maintenanceId = pick(ctx.payload, ['maintenanceId', 'MantenimientoID', 'id']);
  const data = await maintenanceHandlers.get({ ...ctx, payload: { maintenanceId } });
  const title = `Mantenimiento GT - ${data.mantenimiento.Cliente || 'Cliente'}`;
