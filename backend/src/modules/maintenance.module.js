  ...
  return {
    ...
    dispositivos: devices.map((device) => ({
      ...
      Imagenes: images
        ...
        .map((image) => ({ ...image, PreviewURL: image.DriveFileID ? `https://example.invalid/configure-google-service)}&sz=w1200` : image.DriveURL })),
    })),
  };
}
...
export const maintenanceHandlers = {
  ...
  imageUpload: async (ctx) => withImageCreateLock(async () => {
    ...
    if (requestedId) {
      ...
      if (existing) {
        ...
        return { ...existing, PreviewURL: existing.DriveFileID ? `https://example.invalid/configure-google-service)}&sz=w1200` : existing.DriveURL };
      }
    }
  ...
  spreadsheetReport: async (ctx) => {
    ...
    const created = await sheetsApi.spreadsheets.create({ requestBody: { properties: { title: `Mantenimiento GT - ${data.mantenimiento.Cliente || 'Cliente'} - ${String(data.mantenimiento.Fecha).slice(0, 10)}` } } });
    const id = created.data.spreadsheetId;
    const values = [['REPORTE DE MANTENIMIENTO GT'], ['Título', data.mantenimiento.TituloMantenimiento], ['Cliente', data.mantenimiento.Cliente], ['Ubicación', data.mantenimiento.Ubicacion], ['Fecha', data.mantenimiento.Fecha], [], ['Categoría', 'Nombre', 'Zona', 'Fabricante', 'Modelo', 'Serie', 'Funcionamiento', 'En uso', 'Estado', 'Observación'], ...data.dispositivos.map((device) => [device.Categoria, device.NombreDispositivo, device.Zona, device.Fabricante, device.Modelo, device.Serie, device.Funcionamiento, device.EnUso, device.Estado, device.Observacion])];
    await sheetsApi.spreadsheets.values.update({ spreadsheetId: id, range: 'A1', valueInputOption: 'USER_ENTERED', requestBody: { values } });
    const url = `https://example.invalid/configure-google-service}/edit`;
    ...
    return { spreadsheetId: id, spreadsheetUrl: url, excelUrl: `https://example.invalid/configure-google-service}/export?format=xlsx` };
  },

  slidesReport: async (ctx) => {
    if (!isAdmin(ctx)) throw forbidden();
    const data = await enrich(await findById('Mantenimiento', pick(ctx.payload, ['maintenanceId', 'MantenimientoID'])));
    const created = await slidesApi.presentations.create({ requestBody: { title: `Mantenimiento GT - ${data.mantenimiento.Cliente || 'Cliente'}` } });
