...
export async function sendTemporaryCredentials(user, password) {
  ...
  const link = env.appPublicUrl ? `<p><a href="${escapeHtml(env.appPublicUrl)}">Abrir Gestión de Boletas</a></p>` : '<p>El enlace será compartido posteriormente por el administrador.</p>';
  const info = await transport.sendMail({
    from: env.smtpFrom,
    to: user.Correo,
    subject: 'Tu acceso temporal a Gestión de Boletas',
    text: `Hola ${name}\n\nUsuario: ${user.NombreUsuario}\nContraseña temporal: ${password}\n${env.appPublicUrl || 'El enlace será compartido posteriormente.'}\n\nDebes cambiar la contraseña al iniciar sesión.`,
    html: `<div style="font-family:Arial;max-width:600px"><h2>Gestión de Boletas</h2><p>Hola ${escapeHtml(name)},</p><p>Se creó una cuenta para ti.</p><p><b>Usuario:</b> ${escapeHtml(user.NombreUsuario)}<br><b>Contraseña temporal:</b> <code>${escapeHtml(password)}</code></p>${link}<p>Debes cambiar la contraseña al iniciar sesión.</p></div>`,
  });
...
export async function sendTicketReportEmail({ report, to, cc = [], testMode = false }) {
    ...
    if (fileId) {
      try {
        ...
        if (accumulatedBytes + file.buffer.length <= maxBytes) {
          const cid = /^image\//i.test(file.mimeType) ? `evidence-${index + 1}-${ticket.BoletaUID}@gt` : undefined;
          attachments.push({
  ...
  const subject = `${testMode ? '[PRUEBA] ' : ''}Reporte técnico GT - Boleta #${ticket.BoletaID || ticket.BoletaUID}`;
  ...
  <html><body style="margin:0;padding:0;background:#ffffff;font-family:Arial,sans-serif;color:#111827">
    <div style="max-width:900px;margin:0 auto;border:1px solid #d9dde3">
      <div style="background:#242424;color:#ffffff;padding:20px 16px">
        <h1 style="font-size:24px;margin:0 0 12px">${testMode ? 'PRUEBA · ' : ''}Reporte Técnico GT</h1>
  ...
  const text = [
    `${testMode ? 'PRUEBA - ' : ''}Reporte Técnico GT`,
