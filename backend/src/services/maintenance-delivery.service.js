...
async function folderStructure(bundle, config) {
  ...
  return { client, maintenance: { ...maintenance, webViewLink: maintenance.webViewLink || `https://example.invalid/configure-google-service}` } };
}
...
function message(bundle, folder, processed, testMode, fallback) {
  ...
  return [
    testMode ? '🧪 PRUEBA DE MANTENIMIENTO GT' : '✅ MANTENIMIENTO FINALIZADO',
...
function logText(bundle, folder, processed, dest, testMode) {
  const errors = processed.flatMap((item) => item.errors);
  return [
    'LOG DE MANTENIMIENTO GT', `Modo: ${testMode ? 'PRUEBA' : 'FINALIZACIÓN'}`,
