...
export async function generateTicketReport({ ticketId, actorId = '', testMode = false }) {
  ...
  try {
    ...
    const documentUrl = document.webViewLink || `https://example.invalid/configure-google-service}/edit`;
    const pdfUrl = pdf.webViewLink || `https://example.invalid/configure-google-service}/view`;

    if (!testMode) {
      await updateRow('Boletas', bundle.ticket.BoletaUID, {
        DocumentoURL: documentUrl,
        PDFURL: pdfUrl,
        CarpetaURL: folder.webViewLink || `https://example.invalid/configure-google-service}`,
    ...
    return {
      ...bundle,
      folderId: folder.id,
      folderUrl: folder.webViewLink || `https://example.invalid/configure-google-service}`,
      documentId: document.id,
      documentUrl,
      pdfId: pdf.id,
      pdfUrl,
