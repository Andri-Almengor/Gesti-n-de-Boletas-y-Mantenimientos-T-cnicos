...
export default function TicketEvidenceMultiSelectBridge() {
  ...
  useEffect(() => {
    ...
    function summaryNode(form) {
      let node = form.querySelector('[data-gt-multi-evidence-summary]');
      if (!node) {
    ...
    function uploadButton(form) {
      let button = form.querySelector('[data-gt-multi-evidence-upload]');
      if (!button) {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (
        ...
        || !input?.multiple
        || !form.querySelector('[data-gt-multi-evidence-summary]')
        || !form.querySelector('[data-gt-multi-evidence-upload]')
    ...
    return () => {
      ...
      const selectButton = form?.querySelector('.ticket-detail-capture-actions .button[data-gt-multi-label]');
      ...
      detachCurrentListeners();
      detachButtonListener();
      form?.querySelector('[data-gt-multi-evidence-summary]')?.remove();
      form?.querySelector('[data-gt-multi-evidence-upload]')?.remove();
      selectedFilesRef.current = [];
    };
