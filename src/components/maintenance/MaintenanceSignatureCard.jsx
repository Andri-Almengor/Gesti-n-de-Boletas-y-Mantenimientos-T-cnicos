...
export default function MaintenanceSignatureCard({
  ...
  useEffect(() => {
    ...
    window.addEventListener('gt-offline-sync-complete', refresh);
    return () => {
      active = false;
      window.removeEventListener('focus', refresh);
      window.removeEventListener('gt-offline-sync-complete', refresh);
    };
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: 'Abra este enlace para firmar el mantenimiento general realizado por GT. La firma se aplicará a todas las boletas automáticas relacionadas.',
          url,
        });
      } else {
        await copyLink();
