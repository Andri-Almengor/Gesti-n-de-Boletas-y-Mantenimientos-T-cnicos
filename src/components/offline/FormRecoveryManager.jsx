...
export default function FormRecoveryManager() {
  ...
  const persistNow = useCallback(async ({ quiet = false } = {}) => {
    ...
    try {
      ...
      setStatus('local');
      setNotice('');
      window.dispatchEvent(new CustomEvent('gt-form-draft-saved', {
        detail: { key: entry.key, route: entry.route, savedAt: saved.updatedAt },
      }));
      window.clearTimeout(releaseTimerRef.current);
      releaseTimerRef.current = window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('gt-offline-editing-complete', {
  ...
  const restoreControls = useCallback(() => {
    ...
    document.querySelectorAll('input[type="file"]').forEach((input) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('gt-draft-restore-files', {
    ...
    if (data.signature) {
      window.dispatchEvent(new CustomEvent('gt-draft-restore-signature', {
  ...
  useEffect(() => {
    ...
    window.addEventListener('gt-signature-draft-change', captureSignature);
    ...
    return () => {
      ...
      window.removeEventListener('gt-signature-draft-change', captureSignature);
