...
export default function OfflineSyncManager() {
  ...
  const synchronize = useCallback(async ({ forced = false } = {}) => {
    if (!sessionToken || navigator.onLine === false || syncingRef.current) {
      if (forced && navigator.onLine === false) {
        ...
        window.dispatchEvent(new CustomEvent('gt-offline-sync-error', {
    ...
    window.dispatchEvent(new CustomEvent('gt-offline-sync-start', { detail: { forced } }));
    ...
    try {
      ...
      if (!operations.length) {
        ...
        window.dispatchEvent(new CustomEvent('gt-offline-sync-complete', {
  ...
  useEffect(() => {
    ...
    window.addEventListener('gt-offline-editing-complete', releaseEditing);
    ...
    return () => {
      ...
      window.removeEventListener('gt-offline-editing-complete', releaseEditing);
    };
  }, [scheduleSync]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('gt-offline-queue-change', handleQueueChange);
    window.addEventListener('gt-offline-sync-request', handleManualSync);
    ...
    return () => {
      ...
      window.removeEventListener('gt-offline-queue-change', handleQueueChange);
      window.removeEventListener('gt-offline-sync-request', handleManualSync);
  ...
  useEffect(() => {
    document.body.classList.toggle('gt-entity-unsynced', finalizationBlocked);
    return () => document.body.classList.remove('gt-entity-unsynced');
  }, [finalizationBlocked]);
