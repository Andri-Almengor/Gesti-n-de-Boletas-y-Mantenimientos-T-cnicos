...
export default function InstallAppCard() {
  ...
  useEffect(() => {
    ...
    function appInstalled() {
      ...
      setInstalled(true);
      setMessage('Gestión de Boletas se instaló correctamente.');
    }
    window.addEventListener('gt-install-available', promptAvailable);
    window.addEventListener('appinstalled', appInstalled);
    return () => {
      window.removeEventListener('gt-install-available', promptAvailable);
  ...
  return (
    <section className="install-app-card">
      ...
      <div className="install-app-card__content">
        <h2>{installed ? 'Aplicación instalada' : 'Instalar Gestión de Boletas'}</h2>
        <p>{installed
          ? 'La aplicación se está ejecutando desde la pantalla de inicio.'
          : 'Agregue Gestión de Boletas al teléfono o computadora para abrirla como una aplicación independiente.'}</p>
        ...
        {showIosHelp && !installed && (
          <div className="install-app-card__ios-help">
            ...
            <span>1. Abra Gestión de Boletas en Safari.</span>
