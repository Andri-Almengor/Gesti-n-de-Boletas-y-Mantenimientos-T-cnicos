...
export default function TicketVisitGroupPanel({ boletaUid, sessionToken, canCreate, canEdit }) {
  ...
  useEffect(() => {
    ...
    load();
    window.addEventListener('gt-offline-sync-complete', load);
    return () => {
      active = false;
      window.removeEventListener('gt-offline-sync-complete', load);
    };
  ...
  useEffect(() => {
    ...
    refreshQueue();
    window.addEventListener('gt-offline-queue-change', refreshQueue);
    window.addEventListener('gt-offline-sync-complete', refreshQueue);
    window.addEventListener('gt-offline-sync-error', refreshQueue);
    return () => {
      active = false;
      window.removeEventListener('gt-offline-queue-change', refreshQueue);
      window.removeEventListener('gt-offline-sync-complete', refreshQueue);
      window.removeEventListener('gt-offline-sync-error', refreshQueue);
    };
  }, [relationIds]);
  ...
  useEffect(() => {
    document.body.classList.toggle('gt-group-unsynced', groupPending);
    return () => document.body.classList.remove('gt-group-unsynced');
  }, [groupPending]);
