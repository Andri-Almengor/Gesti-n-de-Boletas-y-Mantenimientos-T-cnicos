...
function sanitizeGeminiHtml(html = '') {
  ...
  const documentNode = parser.parseFromString(`<div id="gt-ai-root">${String(html)}</div>`, 'text/html');
  const root = documentNode.getElementById('gt-ai-root');
  if (!root) return '';
  ...
  return <div className="page knowledge-editor-page">
    ...
    <form onSubmit={(event) => save(event, form.status)}>
      ...
      <section className="form-card">
        ...
        <div className="knowledge-video-fields">{form.videos.map((video, index) => <div key={index}><label className="field-group"><span className="field-label">Video {index + 1}</span><input className="form-control" type="url" value={video} onChange={(event) => updateVideo(index, event.target.value)} placeholder="https://example.invalid/configure-service..." disabled={saving} /></label><button type="button" className="icon-button icon-button--outlined" onClick={() => removeVideo(index)} aria-label="Quitar video" disabled={saving}><Icon name="delete" /></button></div>)}</div>
